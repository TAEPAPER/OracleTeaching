2022-0120-01)외부조인(OUTER JOIN)
 - 조인에 참여하는 테이블 중 자료의 종류가 많은쪽을 
   기준으로 적은 테이블에 NULL행을 첨가하여 
   조인을 수행
 - 일반외부조인은 조인조건 기술시 부족한 테이블의 컬럼명
   뒤에 외부조인 연산자 '(+)'를 추가
 - 외부조인 조건이 복수개인 경우 모두 '(+)'연산자를 추가
   해야함
 - 하나의 테이블이 동시에 다수개의 외부조인에 참여할 수 없다
   즉, A,B,C 테이블이 외부 조인에 참여하는 경우 A를 
   기준으로 B테이블이, C를 기준으로 B테이블이 동시에
   외부조인 될 수 없다(A=B(+) AND C=B(+)는 허용되지 않음)
 - 일반조건과 외부조인조건이 동시에 사용되면 정확한 결과를
   얻을수 없다=>서브쿼리로 해결
...
**다음 조건에 맞는 재고 수불테이블을 생성하고 기초 자료를
  입력하시오
  1)테이블명:REMAIN
  2)컬럼
  ----------------------------------------------------
...
(기초자료 입력)
  =>상품테이블의 상품코드와 적정재고를 
    재고수불테이블(REMAIN)의 상품코드와 기초재고에 삽입
    1)년도 : '2005'
    2)상품코드 : 상품테이블의 상품코드
...
사용예)2005년 1월 모든 상품별 매입수량을 조회하여 
      재고수불테이블을 갱신하시오.
...
**상품테이블에 상품별 마일리지를 상품판매가의 0.01%로 
  조정하여 갱신하시오.
  UPDATE PROD
...
**회원테이블의 회원마일리지를 2005년 매출자료를 집계하여
  갱신하시오. 
