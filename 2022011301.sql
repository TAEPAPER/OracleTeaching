2022-0113-01)형변환함수
 1)CAST(expr AS 타입명)
  - 'expr'로 정의된 컬럼 또는 수식의 데이터 타입이 '타입명'
    형식으로 변환

사용예)장바구니테이블에서 2005년 5월 판매정보를 조회하시오
      Alias는 일자,상품명,수량,금액 이며 일자순으로 
      출력하시오
 ...
 2)TO_CHAR(expr[,fmt])
  - 문자열,숫자,날짜 자료를 문자열자료로 형변환   
  - 변환형식을 지정할때에는 'fmt'(형식지정문자열)을 기술
  - 날짜관련 형식문자열
-----------------------------------------------------------
  FORMAT       의미           사용예
-----------------------------------------------------------
 BC, AD    서기, 기원전   SELECT TO_CHAR(SYSDATE,'BC') 
                          FROM DUAL; 
 ...
 AM, PM                      SELECT TO_CHAR(SYSDATE, 'PM')
                               FROM DUAL;
 A.M., P.M.   오전,오후       
 HH,HH12,HH24 시간           SELECT TO_CHAR(SYSDATE, 'WW')
                               FROM DUAL;
 MI           분
 SS           초(01~60)
...
-----------------------------------------------------------
    9     대응되는 무효의 0을
          공백처리            SELECT TO_CHAR(12345,'9999999') AS A1
                               FROM DUAL; 
    0     대응되는 무효의 0을
          '0'으로 출력        SELECT TO_CHAR(12345,'0000000') AS A1
                               FROM DUAL; 
   PR     자료가 음수이면
          < >안에 출력        SELECT TO_CHAR(1234,'99999PR'),
...
사용예)오늘이 2005년 7월31일이고 쇼핑몰 페이지에 처음 로그인
      한 경우 장바구니번호를 생성하시오.  
      
      SELECT TO_CHAR(SYSDATE,'YYYYMMDD')||TRIM(TO_CHAR(1,'00000'))
        FROM DUAL;
        
사용예)오늘이 2005년 7월28일이고 쇼핑몰에 새롭게 로그인
      한 경우 장바구니번호를 생성하시오. 
