2022-0111-01)
 3)LPAD(c1,n[,c2]),RPAD(c1,n[,c2]) 
  - 정의된 기억공간의 크기(n)에서 c1문자열을 채우고 남는공간
    왼쪽(LPAD) 또는 오른쪽(RPAD)에 c2를 채움
  - c2가 생략되면 공백을 채움
  - 수표보호문자 등에 주로 사용되었음

사용예)상품테이블에서 상품명을 출력하되 왼쪽 남는 공간에 '*'를
      채워 출력하시오
...
**회원테이블(MEMBER)의 구조와 데이터를 복사하여 CUSTOMER테이블
  을 생성하고 이름컬럼(MEM_NAME)의 데이터 타입을 
  CHAR(20)으로 변경하시오.
...
--   WHERE MEM_NAME='오철희'; 
 ...
 6)TRIM(c1)
  - 주어진 문자열 c1의 앞과 뒤에 존재하는 모든 무효의 공백
    을 제거
  - 문자열 내부의 공백 제거는 불가능 
...
사용예)CUSTOMER테이블의 이름컬럼의 자료타입을 VARCHAR2(20)
 ...
 7)SUBSTR(c1,n1[,n2])
  ...
  - n2 가생략되면 n1부터 그 이후의 모든 문자열이 추출
  - 추출된 결과도 문자열임
...
사용예)회원테이블에서 20대이하의 회원정보를 조회하시오
 ...
 8)REPLACE(c1,c2[,c3])
  - 주어진 문자열 c1에 포함된 c2문자열을 찾아 c3문자열로 
    치환
  - c3가 생략되면 c2문자열을 제거함 
  - 반복된 c3를 모두 제거
  - 문자열 내부 공백제거에 주로 사용
...
사용예)
  ...
  SELECT REPLACE('APPLEPERSIMPLN','PL','KL')
    FROM DUAL;  
...
사용예)
 ...
 9)ASCII(c1), CHR(n1)
  - ASCII(c1) : c1문자열의 첫 글자만 ASCII 코드값
