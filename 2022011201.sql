...
사용예)
 ...
 3)NEXT_DAY(d1, c)
  - d1 일자이후 가장 먼저 만나는 c요일의 날짜 빈환
  - c 는 요일을 지칭하며 '일요일','일',,'월요일','월'등으로 기술
...
사용예)매입테이블(BUYPROD)에서 2005년 2월 일자별 
      매입집계를 조회하시오
 ...
 5)EXTRACT(fmt FROM d1)
  ...
  - 반환되는 데이터 타입은 숫자
  - 'fmt(Format String: 형식문자열)'은 YEAR,MONTH,DAY,
    HOUR,MINUTE,SECOND 중 하나이어야 함

사용예)회원테이블에서 년령을 구하고 각 년령대별 회원수를 
      조회하시오
      Alias 는 년령대, 회원수
...
사용예) 1998년 3월 10일에 때어난 사람의 정확한 나이는?
