2022-0111-03)숫자함수
 1)수학적 함수
  ...
  - ABS(n1) : n1의 절대값
  - SIGN(n1) : n1의 부호에 따라 양수이면 1, 음수이면 -1, 
    0이면 0을 반환
...
사용예)
 ...
 2)GREATEST(n1[,n2,...]), LEAST(n1[,n2,...])   
   - GREATEST(n1[,n2,...]):제시된 n1[,n2,...]에서 가장 
     큰 값을 반환
   - LEAST(n1[,n2,...]):제시된 n1[,n2,...]에서 가장 작은 
     값을 반환   
사용예)
...
사용예)회원테이블에서 마일리지가 1000이하인 회원의 마일리지를
      1000으로 바꾸어 조회하시오.
 ...
 3)ROUND(n1,l), TRUNC(n1,l)  
   - ROUND(n1,l): 제시된 수 n1에서 소숫점이하 l+1번째 자리
     에서 반올림하여 l자리까지 반환
   - TRUNC(n1,l): 제시된 수 n1에서 소숫점이하 l+1번째 자리
     에서 자리버림하여 l자리까지 반환   
   - l이 생략되면 0으로 간주
   - l이 음수이면 정수(소숫점이상)부분의 l번째자리에서
     반올림(ROUND) 또는 자리버림(TRUNC)
     
 4)FLOOR(n1), CEIL(n1) 
   - FLOOR(n1):n1과 같거나 작은수 중 제일큰 정수  
   - CEIL(n1):n1과 같거나 큰수 중 제일 작은 정수=>소숫점
     이하의 값이 존재하면 무조건 반올림한 값 반환
...
사용예)
 ...
 5)MOD(n1, e), REMAINDER(n1, e)
   ...
   - REMAINDER(n1, e):n1을 e로 나눈 몫의 소숫점이하가
     0.5보다크면(나머지가 e의 중간값 보다 크면) 다음 몫이
     되기위한 값 반환
   - 내부적 처리 방법이 상이
...
사용예)
 ...
 6)WIDTH_BUCKET(val,min,max,b)
   - 범위 하한 값 min에서 상한 값 max를 b개의 구간으로
     나누었을때 제시된 값 val이 어느 구간에 속하는지를 
     판단하여 구간의 index를 반환
     
사용예)회원테이블에서 각 회원들의 마일리지를 입력받아
      마일리지를 1000~9000사이를 9개 구간으로 구분할때
...
사용예)회원테이블에서 각 회원들의 마일리지를 입력받아
      마일리지를 1000~9000사이를 9개 구간으로 구분할때
      회원들의 마일리지가 어느구간에 속하는지 판별하여
      등급을 나타내시오. 단, 가장 많은 마일리지를 가지고있는
      회원이 1등급임
      Alias 회원번호,회원명,마일리지,등급      
      SELECT MEM_ID AS 회원번호,
             MEM_NAME AS 회원명,
             MEM_MILEAGE AS 마일리지,
       --      10-WIDTH_BUCKET(MEM_MILEAGE,1000,9000,9) AS 구간값
