2022-0113-02)NULL처리함수
 ...
 - NULL 관련 함수는 NVL, NVL2, NULLIF와
   데이터가 NULL과 같은지 판단을 위해
   IS NULL, IS NOT NULL, 두 연산자가 제공됨
   
1. IS NULL, IS NOT NULL
 . NULL값과 동등성 평가를 위한 연산자
 . NULL은 '='연산자로 판별 불가능
 
사용예)사원테이블에서 영업실적코드(COMMISSION_PCT)가 
      NULL이 아닌 사원을 조회하시오
...
사용예)상품테이블에서 색상정보(PROD_COLOR)가 존재하지 않는
      상품을 조회하시오
...
2. NULL 처리함수
 1)NVL(col, val)
  - 'col'의 값이 NULL이면 'val'을 반환하고 NULL이
    아니면 'col'값을 반환
